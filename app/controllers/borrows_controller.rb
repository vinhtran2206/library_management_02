class BorrowsController < ApplicationController
  ...
  #accept va deny giong nhau, deu update thang status
  def accept_borrow
    if @borrow.accept!
      update_quantity
      flash[:success] = t ".success"
  ...
  def update_quantity
    @borrow.borrow_details.each do |borrow_detail|
      @borrow.books.each do |book|
        if borrow_detail.book_id == book.id
          book.amount -= borrow_detail.number
  ...
  #kiem tra thoi gian ngay ngay muon nho hon ngay tra
